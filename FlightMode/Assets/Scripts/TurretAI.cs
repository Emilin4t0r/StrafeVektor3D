...
public class TurretAI : MonoBehaviour {
	...
	void Update() {
		...
		barrel.transform.LookAt(player.transform, Vector3.forward);
		...
		if (counter >= fireRate && Vector3.Distance(transform.position, player.transform.position) < range && targetInSight) {
			Shoot();
			counter = 0;
		} else {
